// app/api/exams/[id]/attempt/[attemptId]/questions/route.ts - FIXED
...
export async function GET(
  ...
  try {
    ...
    console.log('Fetching questions:', { examId, attemptId });
    ...
    const formattedQuestions = questions.map(({ item, question }) => {
      ...
      if (exam.shuffleOptions && attempt.shuffleSeed) {
        options = shuffleArray(options, attempt.shuffleSeed + item.questionNumber);
      }

      return {
        id: item.id,
        questionPaperItemId: item.id,
